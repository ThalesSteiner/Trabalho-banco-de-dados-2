...
def ask_user_input() -> dict:
    ...
    print("    3. Criar ambas (original + normalizadas)")
    print("    4. Fazer todas as etapas (original -> normalizadas -> índices)")
    print("    5. Criar apenas índices")
    print("\n  MANUTENÇÃO:")
    print("    6. Deletar tudo (tabelas e schemas)")
    print("\n  OUTRAS:")
    print("    7. Sair")
    print()
    ...
    while True:
        try:
            ...
            if choice in ["1", "2", "3", "4", "5", "6", "7"]:
                ...
                print("Opção inválida. Por favor, escolha 1, 2, 3, 4, 5, 6 ou 7.")
        except (EOFError, KeyboardInterrupt):
            print("\nOperação cancelada pelo usuário.")
            return {"create_main": False, "create_normalized": False, "create_indices": False, "delete_all": False, "all_steps": False}

    options = {
        "1": {"create_main": True, "create_normalized": False, "create_indices": False, "delete_all": False, "all_steps": False},
        "2": {"create_main": False, "create_normalized": True, "create_indices": False, "delete_all": False, "all_steps": False},
        "3": {"create_main": True, "create_normalized": True, "create_indices": False, "delete_all": False, "all_steps": False},
        "4": {"create_main": True, "create_normalized": True, "create_indices": True, "delete_all": False, "all_steps": True},
        "5": {"create_main": False, "create_normalized": False, "create_indices": True, "delete_all": False, "all_steps": False},
        "6": {"create_main": False, "create_normalized": False, "create_indices": False, "delete_all": True, "all_steps": False},
        "7": {"create_main": False, "create_normalized": False, "create_indices": False, "delete_all": False, "all_steps": False},
    }
...
def execute_creation(options: dict) -> bool:
    """Executa a criação de tabelas e índices."""
    if not options["create_main"] and not options["create_normalized"] and not options["create_indices"]:
        return True

    success = True

    # Se for "todas as etapas", garante ordem sequencial
    if options["all_steps"]:
        print("\n" + "=" * 60)
        print("  EXECUTANDO TODAS AS ETAPAS")
        print("=" * 60)

    # ETAPA 1: Criar tabela original primeiro (necessária para normalizadas)
    if options["create_main"]:
        if options["all_steps"]:
            print("\n[ETAPA 1/3] Criando tabela original (o.Tabelona)...")
        else:
            print("\nCriando tabela original (o.Tabelona)...")
        
        ddl_path = os.path.join("scripts", "sql", "ScriptTabelaOriginal.sql")
    ...
    # ETAPA 2: Criar tabelas normalizadas (dependem da tabela original)
    if options["create_normalized"]:
        if options["all_steps"]:
            print("\n[ETAPA 2/3] Criando tabelas normalizadas (schema n)...")
        else:
            print("\nCriando tabelas normalizadas (schema n)...")
        
        ddl_path = os.path.join("scripts", "sql", "DDL1.sql")
        dml_path = os.path.join("scripts", "sql", "DML1.sql")
    ...
    # ETAPA 3: Criar índices (melhora performance)
    if options["create_indices"]:
        if options["all_steps"]:
            print("\n[ETAPA 3/3] Criando índices...")
        else:
            print("\nCriando índices...")
...
def main():
    ...
    # Executar deleção se solicitado
    if options["delete_all"]:
        execute_deletion()
        return

    # Sair se nenhuma opção foi selecionada
    if not options["create_main"] and not options["create_normalized"] and not options["create_indices"]:
